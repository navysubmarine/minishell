...
- a **Lexical Analyzer** = it takes the input characters and puts the characters together into words called tokens (like a big split)
    1) while the input line still exists (browse it with an index) :
    2) make a local copy of it
    3) define the argument delimiters (= SPACE or \t or \n or \r)
    4) define the substring which will be the token

- a **Parser** = it processes the tokens according to a grammar and builds the command table

The **Command Table** is a an array of SimpleCommands structs. A SimpleCommand struct contains members for the command and arguments of a single entry in the pipeline. The parser will also look at the command line and determine if there is any input of ouptut redirection based on symbols present in the command (i.e. < infile, or > outfile).
